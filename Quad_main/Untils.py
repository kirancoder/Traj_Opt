...
def dynamics(vars):
    ...
    for i in range(N):
        um = 0.5 * (u[i] + u[i+1]) + (dt_scaled / 8) * (c1 * U1[i] * jnp.sin(theta[i]) - c1 * U1[i+1] * jnp.sin(theta[i+1])) / m
        vm = 0.5 * (v[i] + v[i+1]) + (dt_scaled / 8) * (c1 * U1[i] * jnp.cos(theta[i]) - c1 * U1[i+1] * jnp.cos(theta[i+1])) / m
        ...
        res = res.at[i].set(x[i+1] - x[i] - (dt_scaled / 6) * (u[i] + 4*um + u[i+1]))
        res = res.at[N+i].set(y[i+1] - y[i] - (dt_scaled / 6) * (v[i] + 4*vm + v[i+1]))
        res = res.at[2*N+i].set(u[i+1] - u[i] - (dt_scaled / 6) * c1 * (U1[i] * jnp.sin(theta[i]) + 4*Um1 * jnp.sin(thetam) + U1[i+1] * jnp.sin(theta[i+1])) / m)
        res = res.at[3*N+i].set(v[i+1] - v[i] - (dt_scaled / 6) * (c1 * (U1[i] * jnp.cos(theta[i]) + 4*Um1 * jnp.cos(thetam) + U1[i+1] * jnp.cos(theta[i+1])) / m - g))
        res = res.at[4*N+i].set(theta[i+1] - theta[i] - (dt_scaled / 6) * c2 * (U2[i] + 4*Um2 + U2[i+1]))
